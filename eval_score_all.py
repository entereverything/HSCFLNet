...
from main_model_all import EnsembleModel
...
def val_for_metric(model, eval_loader, criterion=None, tta=False, input_size=512):
    avg_loss = 0
    val_loss = torch.tensor([0])
    scale = ScaleInOutput(input_size)
    tn_fp_fn_tp = [np.array([0, 0, 0, 0]), np.array([0, 0, 0, 0])]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cuda", type=str, default="1")  # GPU编号
