...
class FSCELoss(nn.Module):
    def __init__(self):
        ...
        weight = None
        # weight = torch.FloatTensor([1 for i in range(19)]).cuda()
    ...
    def _scale_target(targets_, scaled_size):
        ...
        targets = F.interpolate(targets_.float(), size=scaled_size, mode='nearest')
...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def forward(self, feats, labels=None, predict=None):
        ...
        labels = torch.nn.functional.interpolate(labels.float(),
...
class ContrastCELoss(nn.Module, ABC):
    def __init__(self):
        ...
        if self.use_rmi:
            self.seg_criterion = FSAuxRMILoss()
        else:
...
if __name__ == '__main__':
    ...
    input = torch.randn(size=(8, 3, 256, 256))
    outputs = {
        'seg':torch.rand(size=(1, 2, 64, 64)),
        'embed':torch.randn(size=(1, 64, 64, 64)) 
               }
    target = torch.randint(low=0, high=1, size=(1, 1, 64, 64))

    loss = pixel_loss_fun(outputs, target)
