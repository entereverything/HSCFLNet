...
#                  activation_type: str = "relu", **norm_kwargs):
...
#     def forward_baseline(self, x, group_index):
#         """baseline 激活 (ReLU/SiLU/GELU)"""
#         if self.dropout:
#             x = self.dropout(x)
...
#         y = self.base_conv[group_index](act_x)
#         y = act_x + y  # 残差加和
#         y = self.layer_norm[group_index](y)
#         return y
#     def forward(self, x):
...
#             if self.activation_type == "relu-kan":
#                 y = self.forward_relukan(_x.clone(), group_ind)
#             else:
#                 y = self.forward_baseline(_x.clone(), group_ind)
